...
# Environment variable se key uthayega, backup ke liye aapki key bhi set hai
...
def home():
    return jsonify({"status": "KhushiSundari Multi-API Backend running ✅"})
...
def download_post():
    ...
    # Hitting the exact endpoint from your MHT file
    ...
    try:
        ...
        if resp.status_code in (401, 403):
            return jsonify({"success": False, "error": "RapidAPI key is invalid or not subscribed to this specific API."}), 403
            
        if resp.status_code != 200:
            return jsonify({"success": False, "error": f"RapidAPI server returned error code {resp.status_code}"}), resp.status_code

        # Send raw data to frontend to safely extract URLs
